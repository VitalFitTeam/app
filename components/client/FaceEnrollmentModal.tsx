...
export const FaceEnrollmentModal: React.FC<Props> = ({ visible, onClose, onSuccess, onError }) => {
	...
	const handleCapture = async () => {
		...
		try {
			...
			onError(t('faceEnrollment.errors.captureFailed'));
		}
	};
	...
	const handleConfirm = async () => {
		...
		setUploading(true);
		try {
			await enrollFace(capturedImage);
			setCapturedImage(null);
			onSuccess();
		} catch (error) {
			const errorMessage = error instanceof Error ? error.message : t('faceEnrollment.errors.uploadFailed');
		...
		return (
			<Modal visible={visible} animationType="slide" transparent>
				<ThemedView style={styles.container}>
					<Text className='font-body' style={styles.message}>{t('faceEnrollment.permission.message')}</Text>
					<TouchableOpacity onPress={requestPermission} style={styles.button}>
						<Text className='font-body' style={styles.buttonText}>{t('faceEnrollment.permission.button')}</Text>
					</TouchableOpacity>
				...
				<View style={styles.container}>
					...
					{uploading && (
						<View style={styles.uploadingOverlay}>
							...
							<Text className='font-body' style={styles.uploadingText}>{t('faceEnrollment.modal.uploading')}</Text>
						</View>
					)}

					<TouchableOpacity style={styles.closeIcon} onPress={handleClose} disabled={uploading}>
					...
					{!uploading && (
						<View style={styles.previewButtonsContainer}>
							<TouchableOpacity style={styles.retakeButton} onPress={handleRetake}>
								<Text className='font-body' style={styles.retakeButtonText}>{t('faceEnrollment.modal.retake')}</Text>
							</TouchableOpacity>
							<TouchableOpacity style={styles.confirmButton} onPress={handleConfirm}>
								<Text className='font-body' style={styles.confirmButtonText}>{t('faceEnrollment.modal.confirm')}</Text>
							</TouchableOpacity>
						</View>
					)}
				</View>
			...
			<View style={styles.container}>
				...
				<View style={styles.instructionContainer}>
					<Text className='font-body' style={styles.instructionText}>
						{t('faceEnrollment.modal.instruction')}
					</Text>
				</View>
