...
export default function InstructorEditRoutineScreen() {
  ...
  const fetchRoutineAndExercises = useCallback(async () => {
    if (!token || !params.routineId) return;

    try {
      setLoading(true);

      // Fetch the routine details and available exercises in parallel
      const [routineResponse, exercisesResponse] = await Promise.all([
        vitalFitApi.routine.getRoutineById(params.routineId, token),
        vitalFitApi.routine.getExercises(token, { page: 1, limit: 20 }),
      ]);

      setRoutineName(routineResponse.name);
      setRoutineDescription(routineResponse.description || '');
      setRoutineLevel(routineResponse.level as 'Beginner' | 'Intermediate' | 'Advanced');

      // Convert routine exercises to editable format
      if (routineResponse.exercises && routineResponse.exercises.length > 0) {
        const exercisesInput: RoutineExerciseInput[] = routineResponse.exercises.map((ex, index) => ({
          tempId: ex.routine_exercise_id || `temp-${index}`,
          exercise: ex.exercise_details,
        ...
        setExercises(exercisesInput);
      }

      setAvailableExercises(exercisesResponse.data || []);
      setHasMoreExercises(exercisesResponse.data?.length === 20);
    } catch (error) {
      console.error('Error fetching routine:', error);
      showToast(
        'error',
        t('instructor.editRoutine.error') || 'Error',
        t('instructor.editRoutine.errorLoadingRoutine') || 'Could not load routine'
      );
      setTimeout(() => router.back(), 2000);
    } finally {
      setLoading(false);
    }
  }, [token, params.routineId, t, router]);

  useEffect(() => {
    if (token && params.routineId) {
      fetchRoutineAndExercises();
    }
  }, [token, params.routineId, fetchRoutineAndExercises]);

  const fetchExercises = async (page: number = 1, reset: boolean = false) => {
    ...
    try {
      ...
      showToast(
        'error',
        t('instructor.editRoutine.error') || 'Error',
        t('instructor.editRoutine.errorLoadingExercises') || 'Could not load exercises'
      );
    } finally {
      setLoading(false);
  ...
  const handleUpdateRoutine = async () => {
    if (!routineName.trim()) {
      showToast(
        'error',
        t('instructor.editRoutine.error') || 'Error',
        t('instructor.editRoutine.nameRequired') || 'Please enter a routine name'
      );
      return;
    }
    ...
    if (exercises.length === 0) {
      showToast(
        'error',
        t('instructor.editRoutine.error') || 'Error',
        t('instructor.editRoutine.exercisesRequired') || 'Please add at least one exercise'
      );
      return;
    }
    ...
    try {
      ...
      await vitalFitApi.routine.updateRoutine(
        params.routineId,
        {
          name: routineName,
      ...
      showToast(
        'success',
        t('instructor.editRoutine.success') || 'Success',
        t('instructor.editRoutine.routineUpdated') || 'Routine updated successfully!'
      );
      ...
      showToast(
        'error',
        t('instructor.editRoutine.error') || 'Error',
        t('instructor.editRoutine.errorUpdating') || 'Could not update routine'
      );
    } finally {
      setUpdating(false);
    }
      ...
      <SafeAreaView style={{ flex: 1, backgroundColor: '#FFFFFF' }} edges={['top', 'left', 'right']}>
        <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
          ...
          <Text className='font-body' style={{ color: '#6B7280', marginTop: 12 }}>
            {t('instructor.editRoutine.loading') || 'Loading routine...'}
          </Text>
        </View>
      </SafeAreaView>
    );
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: '#FFFFFF' }} edges={['top', 'left', 'right']}>
      ...
      >
        ...
        >
          ...
          >
            {t('instructor.editRoutine.title') || 'Edit Routine'}
          </ThemedText>
        </View>
        ...
        <View style={{ marginBottom: 16 }}>
          <Text style={{ color: '#6b7280', fontSize: 12, marginBottom: 4, fontWeight: '600' }} className="font-body">
            {t('instructor.editRoutine.routineName') || 'Routine Name'}
          </Text>
          <TextInput
            placeholder={t('instructor.editRoutine.routineNamePlaceholder') || 'Enter routine name'}
        ...
        <View style={{ marginBottom: 16 }}>
          <Text style={{ color: '#6b7280', fontSize: 12, marginBottom: 4, fontWeight: '600' }} className="font-body">
            {t('instructor.editRoutine.routineDescription') || 'Description'}
          </Text>
          <TextInput
            placeholder={t('instructor.editRoutine.routineDescriptionPlaceholder') || 'Enter routine description'}
            placeholderTextColor='#9CA3AF'
        ...
        <View style={{ marginBottom: 16 }}>
          <Text style={{ color: '#6b7280', fontSize: 12, marginBottom: 4, fontWeight: '600' }} className="font-body">
            {t('instructor.editRoutine.routineLevel') || 'Level'}
          </Text>
            ...
            {(['Beginner', 'Intermediate', 'Advanced'] as const).map((level) => (
              ...
              >
                <Text
                  ...
                  className="font-body"
                >
                  {t(`instructor.editRoutine.levels.${level.toLowerCase()}`) || level}
                </Text>
              </TouchableOpacity>
            ))}
          </View>
        ...
        <View style={{ marginBottom: 16 }}>
          <Text style={{ color: '#111827', fontSize: 16, fontWeight: '700', marginBottom: 8 }} className="font-heading">
            {t('instructor.editRoutine.exercises') || 'Exercises'}
          </Text>
          ...
          {exercises.length > 0 ? (
            exercises.map((ex) => (
              ...
              >
                ...
                <TextInput
                  placeholder={t('instructor.editRoutine.notesPlaceholder') || 'Notes (optional)'}
            ...
            <View style={{ paddingVertical: 20, alignItems: 'center' }}>
              <Text style={{ color: '#6b7280', fontSize: 14, textAlign: 'center' }} className="font-body">
                {t('instructor.editRoutine.noExercisesYet') || 'No exercises added yet'}
              </Text>
            </View>
          )}
          ...
          >
            ...
            <Text style={{ color: '#f97316', fontSize: 14, fontWeight: '700', marginLeft: 8 }} className="font-body">
              {showExerciseSelector
                ? t('instructor.editRoutine.closeExerciseList') || 'Close'
                : t('instructor.editRoutine.addExercise') || 'Add Exercise'}
            </Text>
          </TouchableOpacity>
          ...
          {showExerciseSelector && (
            <View style={{ marginTop: 12 }}>
              <Text style={{ color: '#111827', fontSize: 14, fontWeight: '700', marginBottom: 8 }} className="font-heading">
                {t('instructor.editRoutine.selectExercise') || 'Select Exercise'}
              </Text>
                ...
                >
                  {loadingMore ? (
                    ...
                    <Text style={{ color: '#f97316', fontSize: 14, fontWeight: '600' }} className="font-body">
                      {t('instructor.editRoutine.loadMore') || 'Load More Exercises'}
                    </Text>
                  )}
                </TouchableOpacity>
              )}
              ...
              {!hasMoreExercises && availableExercises.length > 0 && (
                <View style={{ paddingVertical: 12, alignItems: 'center' }}>
                  <Text style={{ color: '#6b7280', fontSize: 12 }} className="font-body">
                    {t('instructor.editRoutine.allExercisesLoaded') || 'All exercises loaded'}
                  </Text>
                </View>
              )}
            </View>
        ...
        <TouchableOpacity
          ...
          style={{
            ...
            paddingVertical: 14,
            borderRadius: 16,
            backgroundColor: updating ? '#9CA3AF' : '#f97316',
            alignItems: 'center',
            justifyContent: 'center',
            marginBottom: 12,
          }}
        ...
        >
          {updating ? (
            ...
            <Text style={{ color: '#FFFFFF', fontSize: 14, fontWeight: '700' }} className="font-body">
              {t('instructor.editRoutine.updateRoutine') || 'Update Routine'}
            </Text>
          )}
        </TouchableOpacity>
