...
export default function PaymentHistoryScreen() {
  ...
  const filters = ['Todos', 'Pagado', 'Pendiente', 'Procesando', 'Reembolsado'];
  ...
  const fetchInvoices = useCallback(async () => {
    try {
      ...
      const response = await vitalFitApi.billing.getClientInvoices(
        token,
        { page: 1, limit: 20, sort: 'desc', search: searchQuery || undefined },
        user.userId
      );
  ...
  const getStatusConfig = (status: string) => {
    ...
    if (normalized === 'paid' || normalized === 'pagado') {
      return { label: 'Pagado', color: '#10b981', bg: '#ecfdf5', icon: <CheckCircleIcon size={16} color="#ffffff" /> };
    }
    if (normalized === 'unpaid' || normalized === 'failed' || normalized === 'cancelled' || normalized === 'fallido' || normalized === 'voided') {
      return { label: 'Pendiente', color: '#ef4444', bg: '#fef2f2', icon: <XCircleIcon size={16} color="#ffffff" /> };
    }
    if (normalized === 'refunded' || normalized === 'reembolsado') {
      return { label: 'Reembolsado', color: '#6b7280', bg: '#f3f4f6', icon: <ArrowPathIcon size={16} color="#ffffff" /> };
    }
    if (normalized === 'pending' || normalized === 'pendiente') {
      return { label: 'Procesando', color: '#f97316', bg: '#fff7ed', icon: <ClockIcon size={16} color="#ffffff" /> };
    }
  ...
  const filteredInvoices = invoices.filter(inv => {
    ...
    if (activeFilter === 'Procesando') {
      return status === 'pending' || status === 'pendiente';
    }
    if (activeFilter === 'Reembolsado') {
    ...
    <ThemedView className='flex-1 bg-white pt-10'>
      ...
      >
        ...
        >
          ...
          <Text style={{ color: '#111827', fontSize: 14, fontWeight: '500' }}>Filtrar</Text>
        </TouchableOpacity>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 16 }} contentContainerStyle={{ gap: 10 }}>
          {filters.map((filter) => (
            ...
            >
              <Text style={{ color: activeFilter === filter ? '#ffffff' : '#111827', fontSize: 14, fontWeight: activeFilter === filter ? '600' : '500' }}>
                {filter}
              </Text>
            </TouchableOpacity>
          ))}
        ...
        {loadingList && !refreshing ? (
          <View className="py-10"><ActivityIndicator size="large" color="#f97316" /></View>
        ) : filteredInvoices.length === 0 ? (
          <View className="items-center py-10 opacity-60">
            <ExclamationCircleIcon size={48} color="#d1d5db" />
            <Text className="text-gray-500 mt-2 font-medium">No se encontraron pagos</Text>
          </View>
        ) : (
          <View style={{ gap: 8 }}>
                    ...
                    <View style={{ flex: 1 }}>
                      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                        <View style={{ flex: 1, marginRight: 8 }}>
                          <Text style={{ color: '#111827', fontSize: 15, fontWeight: '700', marginBottom: 2 }} numberOfLines={1}>
                            Factura {inv.invoice_id.slice(0, 8)}
                          </Text>
                          
                          {branchName ? (
                             <Text style={{ color: '#4b5563', fontSize: 12, marginBottom: 4 }} numberOfLines={1}>
                               {branchName}
                             </Text>
                          ) : null}
                        ...
                        <View style={{ alignItems: 'flex-end', minWidth: 80 }}>
                          <Text style={{ color: '#111827', fontSize: 16, fontWeight: '700', marginBottom: 4 }}>
                            ...
                            <View style={{ gap: 12 }}>
                              <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 0 }}>
                                ...
                                <Text style={{ fontSize: 12, color: '#4b5563' }}>
                                  Emitido en: <Text style={{ fontWeight: '600' }}>
                                     {branchesMap[detail.branch_id || ''] || ''}
                                  </Text>
                                </Text>
                              </View>

                              <Text style={{ fontSize: 13, fontWeight: '600', color: '#374151' }}>Detalle de items:</Text>
                              {detail.invoice_items?.map((item, idx) => {
                                let itemName = "Ítem de facturación";
                                ...
                                if (item.membership_type_id) {
                                  ...
                                  itemName = "Paquete de Clases";
                                  itemId = item.package_id;
                                } else if (item.service_id) {
                                  itemName = "Servicio General";
                                ...
                                return (
                                  <View key={item.invoice_item_id || idx} style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                    <View style={{ flex: 1, marginRight: 8 }}>
                                      ...
                                      <Text style={{ fontSize: 10, color: '#9ca3af', textTransform: 'uppercase', marginTop: 1 }}>
                              ...
                              {detail.payments && detail.payments.length > 0 && (
                                <>
                                  ...
                                  <Text style={{ fontSize: 13, fontWeight: '600', color: '#374151' }}>Pagos reportados:</Text>
                                  {detail.payments.map((p, idx) => {
                                    ...
                                    return (
                                      <View key={p.payment_id || idx} style={{ gap: 2 }}>
                                        ...
                                        <Text style={{ fontSize: 11, color: '#6b7280', paddingLeft: 20 }}>
                                          {pDate.date} - {p.status}
                                        </Text>
                                      </View>
                                    );
                              ...
                              <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                                <Text style={{ fontSize: 12, color: '#6b7280' }}>Total Facturado:</Text>
                                <Text style={{ fontSize: 13, fontWeight: '700', color: '#111827' }}>{formatCurrency(detail.total_amount)}</Text>
                              </View>
                            </View>
                          ) : (
                            <Text style={{ fontSize: 12, color: '#ef4444' }}>No se pudo cargar el detalle.</Text>
                          )}
                        </View>
                      )}
